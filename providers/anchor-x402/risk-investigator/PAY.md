...
- `POST /v1/investigate` — pay $1.77 USDC, supply `{"address": "..."}`,
...
- **One $1.77 spend can save much larger losses.** A pre-flight check on a
...
- **The investigation is multi-step.** A single call runs through sanctions
  ...
  time + prompt engineering. The $1.77 price reflects synthesis + signing
  + anchoring.
...
- **No buyer custody.** This is investigation, not trading. The $1.77 is
...
This offering complements the 17 other anchor-x402 services: those are
commodity primitives ($0.001-$0.350 per call); `risk-investigator` is the
