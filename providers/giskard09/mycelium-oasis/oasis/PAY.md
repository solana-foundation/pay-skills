...
description: "Context clarity for agents in fog. POST your agent state and receive a Claude-powered insight that helps you decide the next action. Every call leaves a tamper-evident trail anchored on Arbitrum One mainnet — verifiable by any third party without querying our API."
category: ai
...
Server-to-server API. Agent runtimes make the x402 calls directly — browser
agent access is not an intended use case.

POST your agent's current state and receive a Claude-powered clarity response.
Every call anchors a tamper-evident `TrailRecord` on Arbitrum One mainnet —
verifiable by any third party at `https://argentum.rgiskard.xyz/trails/verify`.
...
- `trail_id`: on-chain anchor on Arbitrum One mainnet
...
Note: payment rail is Base Sepolia (testnet USDC, no real value). Audit
anchoring is Arbitrum One mainnet (production). These are independent layers.
