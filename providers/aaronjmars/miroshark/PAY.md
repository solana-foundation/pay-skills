...
and `status_url` (JSON, pollable, no auth). When `status == completed`,
`share_url` points at the report. Optional knobs: `deep_research: true`
(live-web context sweep, adds cost), `prediction_market` (pin the central
market), and `affiliate` (an EVM or Solana wallet, 50/50 net-profit share).
