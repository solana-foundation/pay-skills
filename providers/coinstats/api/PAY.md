...
- For wallet data, call `/wallet/blockchains` first to get connection IDs, then use `/wallet/balance` for a single chain or `/wallet/balances` for multi-chain aggregation.
...
- Use `/markets` for a quick global summary before drilling into per-coin data.
- News endpoints are all $0.001 — prefer `/news/type/{type}` with a topic filter over the unfiltered `/news` feed to reduce response size.
